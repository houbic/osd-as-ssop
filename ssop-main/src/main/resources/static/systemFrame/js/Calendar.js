...
var bMoveable=true;
var strFrame;		
//document.writeln('<iframe id=endDateLayer frameborder=0 style="position: absolute; width: 162; height: 211; z-index: 9998; display: "></iframe>');
document.writeln('<iframe id=endDateLayer onload="this.height=211;this.width=162" frameborder=0 style="position: absolute; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
var needFullDateTime = false; //控件初始化时是否显示时间

//odatelayer.bUseTimeLayer.innerText="NO";
bImgSwitch();
odatelayer.bUseTimeLayer.innerHTML=bImg;

function setFullTime(tt, hasTime)
{
    needFullDateTime = hasTime;
    setday(tt);
}
...
function setday(tt, obj) //主调函数
{
    ...
    // 显示时间
    if (outObject.value.length>10 || needFullDateTime)
	{
		bUseTime=true;
		bImgSwitch();
		odatelayer.bUseTimeLayer.innerHTML=bImg;
...
var MonHead = new Array(12);                       //定义阳历中每个月的最大天数 
		...
		{ 
			...
			removeAll(dayObj);
			dayObj.options.add(new Option("--请选择--",""));
			...
			if (dayObj.value<'1') dayObj.value="";
			...
			//dayObj.options[1].selected = true;
        }   
         //移除
